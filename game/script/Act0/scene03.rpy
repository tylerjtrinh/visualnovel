label a0s3:
    scene bg starry_sky with fade
    a normal "Gonna be a long, long drive home."
    v normal "Well, you got here, so it’ll be easy enough to get back."
    a "I guess."
    v "Anyway, we were talking about our dreams…"
    a "Got something new to add?"
    v "Hmmmm…"
    v "I mean, what dreams do you have?"
    a "Well, what we’ve got right now is great. Don’t think I need much more than that."
    v "Mhmm."
    v "An answer, but not your own."
    v "So tell me, Alex, and be honest… What is your dream?"
    a "I just told you, I’m happy with what we have. I don’t need any more dreams."
    v "I don’t believe you."
    a "Why not? Can’t I be happy where I am now?"
    v "...It’s getting late. Maybe we should start heading back."
    a "Val… you seem upset."
    v "{i}I’m not.{/i}"
    a "Is this about the dream thing?"
    v "..."
    a "I meant what I said, you know. My dreams are fulfilled! We’re married, we have a beautiful home… What else do we need?"
    v "......"
    a "...You wanted a different answer, didn’t you?"
    v "No, no… There’s no wrong answer."
    a angry "Feels like I found one."
    v "Well, it’s just that I wanted you to… I hoped you’d say something about…"
    stop music
    scene black with in_glitch
    show screen title_card_display("Say IT!")
    pause 1
    hide screen title_card_display
    scene bg starry_sky with dissolve 
    v sad "...It doesn’t matter. We can talk about it later."
    a slightsmile "No, something’s on your mind. Just tell me."
    v "It’s not something I can just spit out-"



    
    
    
